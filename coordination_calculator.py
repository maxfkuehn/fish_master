...
if __name__ == "__main__":
    ...
    if b == 0:
        ...
        np.save(laod_path + record_day + '/x_coordinates.npy', np.array(x_coordinate_af, dtype=object))
        np.save(load_path + record_day + '/y_coordinates.npy', np.array(y_coordinate_af, dtype=object))
        np.save(load_path + record_day + '/times_coordinates.npy', np.array(unique_time_points_af, dtype=object))



    # Project the coordinates of themiddlelinesof grid
    else:
        x_coordinates = np.load('x_coordinates.npy', allow_pickle=True)
        y_coordination = np.load('y_coordinates.npy', allow_pickle=True)
        timepoints = np.load('time_coordinates.npy', allow_pickle=True)
    ...
    m4 = ((middle_line_points[4][1] - middle_line_points[0][1]) / (
            middle_line_points[4][0] - middle_line_points[0][0]))
    m3 = ((middle_line_points[5][1] - middle_line_points[4][1]) / (
            middle_line_points[5][0] - middle_line_points[4][0]))
    m2 = ((middle_line_points[8][1] - middle_line_points[5][1]) / (
            middle_line_points[8][0] - middle_line_points[5][0]))
    m1 = ((middle_line_points[15][1] - middle_line_points[8][1]) / (
            middle_line_points[15][0] - middle_line_points[8][0]))
    ...
    #get point on line
    #oint1 = [0,line_equation(0,m1,intercept1)]
    ...
    for idx in range(len(x_coordinates)):
        ...
        for x_co,y_co in zip_coordinates:
           ...
           #capbottom part
           if x_co >= middle_line_points[0][0]:
               pro_x.extend([middle_line_points[0][0]])
               pro_y.extend([middle_line_points[0][1]])
           #cap top part
           elif y_co >= middle_line_points[-1][1]:
               pro_x.extend([middle_line_points[-1][0]])
               pro_y.extend([middle_line_points[-1][1]])
           #on sectionborder 1
           elif y_co == line_equation(x_co, sector_m1, sector_b1):
                print('sector border point 1')
           # on sectionborder 2
           elif y_co == line_equation(x_co, sector_m2, sector_b2):
                print('sector border point 2')
           # on sectionlborder 2
           elif y_co == line_equation(x_co, sector_m3, sector_b3):
               print('sector border point 3')
           # on section 1
